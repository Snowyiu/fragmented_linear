...
class FragmentedLinear(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            # Get selected fragments and their corresponding weights
            selected_fragments = x_fragments[batch_idx, selected_idx]  # Shape: [batch_size, fragment_size]
            selected_weights = self.expert_weights[selected_idx]      # Shape: [batch_size, fragment_size, out_features]
            
            # Do matrix multiplication for each item in batch
            expert_outputs = torch.bmm(
                selected_fragments.unsqueeze(1),  # Shape: [batch_size, 1, fragment_size]
                selected_weights                  # Shape: [batch_size, fragment_size, out_features]
            ).squeeze(1)  # Final shape: [batch_size, out_features]
