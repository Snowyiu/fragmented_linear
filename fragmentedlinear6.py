...
class FragmentedLinear(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            expert_outputs = torch.zeros(batch_size, self.out_features, device=x.device)
            
            # Process each fragment
            for fragment_id in range(self.num_fragments):
                # Find batch items that selected this fragment
                fragment_mask = (selected_idx == fragment_id)
                if not fragment_mask.any():
                    continue
                    
                # Get the batch items and corresponding fragments
                batch_indices = fragment_mask.nonzero().squeeze(-1)
                current_fragments = x_fragments[batch_indices, fragment_id]
                
                # Process these batch items with the fragment's expert
                fragment_output = torch.mm(
                    current_fragments,  # [num_selected, fragment_size]
                    self.expert_weights[fragment_id]  # [fragment_size, out_features]
                )
                
                # Place results in output tensor
                expert_outputs[batch_indices] = fragment_output
